...
class FileSubjects(TypedDict):
    ...
    on_save_scene: Subject[None]
    on_load_scene: Subject[None]
    on_save_render_result: Subject[None]
...
class WindowOpenSubjects(TypedDict):
    ...
    renderer_config_open: BehaviorSubject[bool]
...
class RendererState(TypedDict):
    ...
    always_build_bvh: BehaviorSubject[bool]
            ...
            Menu(
                ...
                children=[
                    ...
                    MenuItem(
                        device=self._device,
                        adapter=self._adapter,
                        name="Always Build BVH",
                        open=kwargs["renderer_state"]["always_build_bvh"],
                        on_open_changed=lambda opened: kwargs["renderer_state"][
                            "always_build_bvh"
                        ].on_next(opened),
                    ),
                    SimpleMenuItem(
                        ...
                        adapter=self._adapter,
                        name="Config",
                        on_clicked=lambda: kwargs["window_open_subjects"][
                            "renderer_config_open"
                        ].on_next(True),
                    ),
                    BVHMenuItem(
                        device=self._device,
